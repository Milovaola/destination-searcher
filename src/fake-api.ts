...
/**
 *
 * A function that returns destinations or an error.
 * @param value - User entered value.
 */
export const getDestinations = (value: string): Promise<DestinationEntity[]> => {

  console.log(value, 'getDestinations function arguments')

  if (value === 'fail') {
    return Promise.reject({
      code: 500,
      message: 'An error occurred during the request. Please try again later',
    })
  } else {
    const res = destinations.filter(({ name }) => name.toLowerCase().includes(value.toLowerCase()))

    return Promise.resolve(res)
  }
}

/** Get a full list of destination points for search nearby destinations. */
export const useDestinationList = () => useQuery({
  queryKey: 'fullListDestinations',
  queryFn: () => Promise.resolve(destinations),
})
